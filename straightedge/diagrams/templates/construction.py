...
class ConstructionTemplate:
    ...
    def refusal_findings(self, params: Dict[str, Any]) -> List[Finding]:
        """Why this template refused to draw — claim failures only.

        A claim the construction does not satisfy blocks the drawing
        deliberately, and the caller should hear which claim. A construction
        that could not be *built* — a notation error, an unknown id — is a
        parameter problem after all, and stays reported as one.

        Asked through :func:`straightedge.diagrams.registry.refusal_findings`,
        which the MCP `draw` tool and the CLI `draw` command share. It lived in
        the MCP server first, which left the CLI reporting a parameter mistake
        for a construction whose parameters were fine.
        """
