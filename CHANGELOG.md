...
- `site/posts/feed.xml`, an Atom feed, generated by `tools/build_blog_feed.py`
  ...
  summary from the description meta tag. A feed maintained beside the posts is
  exactly the file that rots: nothing renders it during review, and the first
  person to notice it is stale is a subscriber. `--check` fails if it is out of
  date. A post missing any of the three fields is reported rather than skipped.
